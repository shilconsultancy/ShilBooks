...
function formatPercentageChange($current, $previous) {
    ...
    return '<span class="text-xs text-'.$color.'-500 inline-flex items-center"><i data-feather="'.$icon.'" class="w-3 h-3 mr-1"></i>' . round(abs($percentage)) . '%</span>';
}
...
$profit_current_quarter = $payments_received_current_quarter - $expenses_current_quarter;
...
$profit_previous_quarter = $payments_received_previous_quarter - $expenses_previous_quarter;
...
$upcoming_expenses = [];
$recurring_sql = "SELECT * FROM recurring_expenses WHERE status = 'active'";
$recurring_stmt = $pdo->prepare($recurring_sql);
$recurring_stmt->execute();
while ($profile = $recurring_stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    $profile['next_due_date'] = $last_gen;
    $upcoming_expenses[] = $profile;
}
usort($upcoming_expenses, function($a, $b) {
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-macgray-200">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="mt-4 md:mt-0 text-right">
                        <div id="clock" class="text-3xl font-bold text-macblue-600"></div>
                        <div id="date" class="text-sm text-macgray-500"></div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 border border-macgray-200">
                    ...
                    <div class="space-y-3">
                        <?php if (empty($upcoming_expenses)): ?>
                            <p class="text-sm text-macgray-500">No upcoming recurring expenses.</p>
                        <?php else: ?>
                            <?php foreach (array_slice($upcoming_expenses, 0, 5) as $expense): ?>
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="font-medium text-macgray-800"><?php echo htmlspecialchars($expense['description']); ?></p>
                                    <p class="text-xs text-macgray-500">Due: <?php echo $expense['next_due_date']->format('M d, Y'); ?></p>
                                </div>
                                <p class="font-semibold text-macgray-800"><?php echo CURRENCY_SYMBOL; ?><?php echo number_format($expense['amount'], 2); ?></p>
                            </div>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </div>
