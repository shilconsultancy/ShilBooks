...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) as count FROM sales_receipts WHERE user_id = :user_id");
            $stmt->execute(['user_id' => $userId]);
            $receipt_count = $stmt->fetchColumn();
            $receipt_number = 'RCPT-' . str_pad($receipt_count + 1, 4, '0', STR_PAD_LEFT);

            // 2. Insert into the main `sales_receipts` table
            $sql = "INSERT INTO sales_receipts (user_id, customer_id, receipt_number, receipt_date, subtotal, tax, total, notes) 
                    VALUES (:user_id, :customer_id, :receipt_number, :receipt_date, :subtotal, :tax, :total, :notes)";
            ...
            $pdo->commit();
            header("location: " . BASE_PATH . "sales/receipts/");
            exit;

        } catch (Exception $e) {
            $pdo->rollBack();
            $errors[] = "Error creating sales receipt: " . $e->getMessage();
        }
    }
}
...
<div class="flex-1 flex flex-col overflow-hidden">
    <header class="bg-white border-b border-macgray-200 py-3 px-6">
        <h1 class="text-xl font-semibold text-macgray-800">Create New Sales Receipt</h1>
    </header>
    ...
    <main class="content-area flex-1 overflow-y-auto p-6 bg-macgray-50">
        <div class="max-w-4xl mx-auto">
            ...
            <form action="create.php" method="POST" id="receipt-form">
                <div class="bg-white p-6 rounded-xl shadow-sm border border-macgray-200">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="receipt_date" id="receipt_date" value="<?php echo date('Y-m-d'); ?>" required class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 flex justify-end"><button type="submit" class="px-6 py-2 bg-macblue-500 text-white font-semibold rounded-md hover:bg-macblue-600">Save Receipt</button></div>
            </form>
        </div>
    </main>
</div>
